    ...
    <div class="content-title">
      <div class="titlename" style="overflow: hidden">
        <Tag color="blue" style="float: right;margin-right:30px;width:100px;height:30px;font-size:1.0em;line-height: 30px;">
          <router-link to="/classAdd" tag = "span">添加课程</router-link>
        </Tag>
      </div>
    </div>
    <div class="content">
      ...
      return {
        companyCol: [
          ...
          },
          {
            ...
            key: 'action',
            render (row) {
              return '<i-button type="text" size="small" @click = "update(row.id)">查看课程章节</i-button>' +
                '<i-button type="text" size="small" @click = "update(row.id)">添加课程章节</i-button>'
            }
          },
          {
            title: '操作',
